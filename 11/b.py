...
def count_adjecent(seats, x: int, y: int, seat) -> int:
    dirs = [lambda x,y: (x-1,y), lambda x,y: (x-1,y-1), lambda x,y: (x,y-1), lambda x,y: (x+1,y-1), lambda x,y: (x+1,y), lambda x,y: (x+1,y+1), lambda x,y: (x,y+1), lambda x,y: (x-1,y+1)]
    count = 0
    for dir in dirs:
        xp, yp = x, y
        while True:
...
def update_seats(source_seats, target_seats, x: int, y: int) -> int:
    ...
    if seat == "L" and count_adjecent(source_seats, x, y, "#") == 0:
        ...
        change_count += 1
    elif seat == "#" and count_adjecent(source_seats, x, y, "#") >= 5:
